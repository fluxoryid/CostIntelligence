...
- [x] Post-deploy runtime reports release channel `production`, build `production-2.1-20260919-v12`, and health `healthy`.
- [x] Provider smoke probes return HTTP 200 for active Production 2.1 sources: JISDOR, BI-Rate, Kurs Pajak, BPS and ESDM.

- [x] Cloudflare production deployment returns Worker build `production-2.1-20260919-v12` and post-deploy `/api/version` verification passes.
...
- [x] Self-service password recovery flow implemented in the production login shell.\n- [x] Dedicated mobile-safe password reset page implemented with native navigation.\n- [x] Native forgot-password link interception removed from legacy dialog handler.\n- [x] Canonical password reset route `/password-reset` used for recovery callbacks.\n- [x] Login password field DOM IDs restored and protected by regression test.\n- [x] CalcCore browser export present and protected by regression test.\n- [x] Session validation and application-startup errors separated.\n- [x] Reset HPS zeroes all active numeric parameters and HPS headline outputs without deleting server history/outcome evidence.\n- [x] Supabase-only persistent system of record enforced; legacy browser localStorage business/workflow/learning persistence removed.\n- [x] Cloudflare Worker remains stateless; no server-side business-data persistence.\n- [ ] Supabase Auth Site URL / Redirect URL must include the production Cloudflare URL and `/password-reset` before recovery emails are reissued.\n- [x] Business owner authorized promotion to Production 2.1 on 19 Sep 2026.
